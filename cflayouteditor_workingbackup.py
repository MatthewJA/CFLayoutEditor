...
class Main(object):
    ...
    def selectComicAndRun(self, function, caller):
        ...
        if 'You are not logged in' in r and '<div class="ehead">' in r:
            ...
            top.title("Comic Select")
            if comics:
                instruction = Label(top, text='Select a comic.')
                instruction.grid(row=0, sticky=W)
                var = StringVar(top)
                var.set("")
                options = OptionMenu(top, var, *comicnames)
                options.grid(row=1)
                b = Button(top, text="I'm cool with this, yo", command=lambda t=top, m=var, c=comics: function(m,t,c))
                b.grid(row=2)
            else:
    ...
    def cf_login(self, incorrect=False, caller=None):
        ...
        submitbutton.configure(command = lambda u=usernameentry,p=passwordentry,b=top,c=caller:self._dologin(b,u,p,c))
    ...
    class Textbox(Text):
        ...
        class SyntaxText(Text):
            ...
            def __init__(self, *args, **kwargs):
                ...
                self.tag_configure("cf_v",foreground="#47DB42")
                self.tag_configure("cf_c",foreground="#25C7B7")
            ...
            def updatetags(self, data):
                ...
                self.highlight('<[^<]+?>', 'blue')
                self.highlight('".*?"', 'red')
                self.highlight('\[v:.*?\]', 'cf_v')
                self.highlight('\[[cl]:.*?\]', 'cf_c')
                self.highlight('\[/\]', 'cf_c')
